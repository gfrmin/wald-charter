...
- **V2.7** `falsifiers([[draws, end, after], ...])`, only with `counts`, ships the falsifying records of C2.J26: one for each plate the Counts passed through that ended falsified, so a chain of refits ships several. Each is a prefix `[draws, None, None]` ending at the report that falsified, or a full record whose after-report falsified; each at most once. A full record with no after-report falsified nothing, and C2.S13 refuses it PLATE: this page decides so, being later than C2.S13, which named no such case [K28]. Whether the Counts and falsifying records together are possible is C2.S13's. Refused MISSING, NOT_A_DECLARATION, DUPLICATE.
- **V2.8** `score(value, of="counts", source="data")`, only with `counts`, declares the Score: for every copy of every record of the Counts, **and for every falsifying record**, its likelihood under the prior conditioned on all the others, multiplied together; the kernel recomputes it, and a pack whose Score differs C2.S14 refuses UNSCORED. Where C2.S14 gives the falsifying records no term, this page decides, being the later, as SURFACE v0.1 K15 did for CHARTER v0.1's missing RATE: everything the prior conditions on is scored [K28]. Refused MISSING, DUPLICATE.
...
| V2.7 | DUPLICATE, MISSING, NOT_A_DECLARATION | 3 | falsified_refit, prefix_falsifier, refit_scored_falsifier |
| V2.8 | DUPLICATE, MISSING | 2 | appendix_a_shipped, monitor_shipping, refit_scored_falsifier |
...
- Session 5, on draft 10 (2026-09-24): every claim reproduced, the five vectors encoded through V2.13's table. Categories 1, 2, 3 and 5: nothing. One finding, 4.1: V2.7 and V2.8 took their meaning from `ERRATA.md`, which is not a signed page, so on the pages that bind, signed C2.S13 and C2.S14 gave every pack shipping a falsifying record the opposite verdict — the page's own fourth vector, shipped in a refit, had three possible Scores (363/10000 by the draft, 3/8 or 33/100 by signed S14), and a full-record falsifier with no after-report was accepted by signed S13 and refused by the draft. Handled gate-first: `page_check.py` gains T6, which refuses a rule that names an unsigned file as the source of what it says; it failed on draft 10. Then V2.7 and V2.8 state the corrected rules in full, and claim precedence as the later page, as SURFACE v0.1 K15 did. No verdict, pack or number changes: the reference already implemented the corrected rules. The history of the correction stays in `ERRATA.md`, which now records it as in force from `surface-v0.2`.
