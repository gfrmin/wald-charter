...
Marks **[K12]–[K18]** are author judgements, ruled in §7.
...
| `score(number, of="fraction"\|"cost", source="data")` | the Score of one `fitted` meta-table, named by `of`: the held-out score of that fit, as measured and written as a rational — a log score is irrational, so the rounding is part of the measurement and the scoreboard prints the Score as written. Its source is `data`; any other is `TABLE_SOURCE`. One per fitted table, no more (`DUPLICATE`) and no fewer: a fitted Fraction or Cost without its Score is `UNSCORED`; a Score of a table that is not `fitted` is `MISSING`. **[K14]** |
...
The World the checker builds gains `dplus`, `fraction`, `rate`, `ops`, `table_sources.dplus`, `.fraction`, `.cost`, `.rate`, and `score` as `{"fraction": number, "cost": number}` for the fitted tables (INTERFACE.md, kit v0.8). `declare` then applies CHARTER v0.1's own refusals: `FRACTION` (f outside [0, 1]), `COST` (a cell below 0), `DEPTH_PLUS` (d ≠ 1, or Depth⁺ ≠ 2, or N < 2, alongside a `think`) — and `RATE` for r below 0, a name this page supplies: CHARTER v0.1 writes r ∈ ℚ≥0 and names no clause for it, so the surface names one, under the name the Rate row already uses for its source **[K15]**.

**Stricter than the charter, by name [K18].** CHARTER v0.1 S3 lets every table name any of the three sources; this page fixes two: Depth⁺ is `elicited` and Score is `data`, anything else `TABLE_SOURCE`. A World with a `data` Depth⁺ or an `elicited` Score is a World under the charter and has no pack here, as v0's K3 already made numerals outside tables unsayable that CHARTER S3 tolerates. Depth⁺ is the owner's and J11 fixes it; a Score is a measurement or it is nothing.
...
| K15 | the surface does not repeat `declare`'s range and depth checks under its own names where the kernel has one; where CHARTER v0.1 has none (r below 0) the surface supplies RATE, and CHARTER v0.2 is to adopt it | | |
...
| K18 | the surface is stricter than CHARTER v0.1 on two sources: Depth⁺ `elicited`, Score `data`; Worlds outside this are unsayable by design (K3's precedent) | | |
...
- Session 3, on draft 3 (2026-09-21): categories 1, 3, 5 clean (every laundering route lands on a name). 3 findings: 2a/4b (the surface is stricter than CHARTER v0.1 on the sources of Depth⁺ and Score, which the charter never names), 4a (RATE for r < 0 rested on a text outside the four pages), 2b (a log score is irrational, so the Score cell cannot hold the measurement as measured). Resolved in draft 4 by naming, not by changing: K18 states the stricter sources as the surface's own judgement with K3 as precedent; K15 amended so the page itself supplies RATE; the Score row says the rounding is the measurement's. The checker is unchanged: it already refused every pack of the session. No act of any admitted pack changed.
