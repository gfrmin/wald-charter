# Attack prompt for SURFACE.md — paste into a fresh session that has seen nothing but CHARTER.md, SURFACE.md and the packs under laws/packs/ok (attach all nine)
...
1. **Smuggling.** A pack the page accepts that says something CHARTER forbids: a choice (E5), a gate or comparison on a probability (S1), an update that is not `condition`, an unhoused or unsourced numeral (S3), a kernel whose rows do not sum to 1 (S4), double-counted evidence (S2), silently handled zero evidence (S5). Show the pack in full and the CHARTER sentence it defeats. The newest text is `reads` (K9), `host` with parameters and its no-number rule (K4), the source rule for parameters (K6), Ω as what the prior names (K10), and the four passes of §5 (K7). Attack those first.
...
5. **Wrong name.** A pack the page refuses, but under two different names depending on how §5's "first rule broken" is read.

Format of every finding: the pack in full; the sentences at issue, quoted; the two verdicts, Worlds or acts, exact. A finding without a complete pack will be discarded unread.
