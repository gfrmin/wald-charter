# SURFACE v0 — draft 2, for the author's ruling
...
| `prior(table, source=…)` | P₀. **Ω is the set of states the prior names** **[K10]**: a state of the product it leaves out is not in Ω. Every other table over states comes after the prior and is total over Ω. |
...
**`reads` [K9].** Every act says what it reads: a non-empty list of components of the space and, for a `once` act, named private sources. The kernel may depend on no component outside `reads`: the checker verifies it. A source named by two acts, or named by a `fresh` act, must be a component (CHARTER S2). What the page cannot check is whether two instruments share something the pack never named; it can only make the pack say what it claims.

## 3. Numbers (CHARTER S3)

A number appears only in a table cell, a mixture weight, a host's parameters, a `param`, a `price`, the `horizon` or the `depth`. A cell is an integer, a ratio of integers, a parameter, or `+ − * /` and unary minus over these **[K2]**. No decimals: `0.9` is refused, `9/10` is lawful. Division by zero is refused. A number anywhere else is refused, 0 and 1 included **[K3]**. Every table names its `source`: `data`, `elicited` or `fitted`, and **reads only parameters of its own source**. A parameter nobody reads is refused. The checker returns the count of numerals by source, which scoreboards print.
...
| `host(name, parameter, …, source=…)` **[K4]** | a deterministic instrument: a pure function from the state and the listed parameters to an outcome, registered by the host under `name`. Its rows sum to 1 by construction. It sees a state, never a belief, a utility or the menu. Its source code may contain no number other than 0 and 1: a number it needs is a `param` of the pack. The World records a fingerprint of its source code. |
...
Of the surface: `SYNTAX`, `NOT_A_DECLARATION`, `DUPLICATE`, `BAD_NAME`, `UNKNOWN_NAME`, `UNKNOWN_HOST`, `FLOAT`, `DIVISION_BY_ZERO`, `UNHOUSED_NUMERAL`, `TABLE_SOURCE`, `UNDECLARED_READ`, `MISSING`, `UNREAD_PARAMETER`. Of the World, from `declare`: `EMPTY_T`, `TABLE_SHAPE`, `PRIOR`, `KERNEL_ROW`, `PRICE`, `DEPTH`, `ZERO_EVIDENCE`, `SHARED_SOURCE`.

**Which name [K7].** Four passes; the first pass that fails names the refusal. (1) The whole text parses, or `SYNTAX`. (2) The declarations are read top to bottom and each left to right; the first offence met names the refusal, including a short mixture, garbling or `by` row met on the way. (3) The pack as a whole: `MISSING`, then `UNREAD_PARAMETER`. (4) The World: `declare`'s names, in the order listed above.
...
| K4 | **amended.** `host(name, parameters…, source=…)`: deterministic, sourced, no number in its code, fingerprinted; stochastic hosts are out of v0 | | |
| K5 | `data` files for `data` and `fitted` tables only; `elicited` numbers live in the pack | accept | 2026-09-20 |
| K6 | **replaced.** Each table names its own source and reads only parameters of that source; a kernel may draw on several; horizon and depth are sourced separately | | |
| K7 | **reworded.** Four passes; the first failing pass, and within it the first offence in reading order, names the refusal | | |
| K8 | out of v0: numeric latent components, generated tables, stochastic hosts | accept | 2026-09-20 |
| K9 | **new.** `reads` is mandatory; the kernel may depend on no component outside it; undeclared sharing in the world is beyond the page | | |
...
- Session 2, on draft 2: ___ claimed, ___ reproduced, all resolved: yes / no.
