# SURFACE v0 — draft 5, for the author's ruling
...
| K6 | **replaced.** Every table and `param` names its source; a parameter keeps its own source wherever read; only `fitted` is fenced (no promotion in v0); the count is of quantities, not numerals; horizon and depth are sourced separately | | |
...
- Session 4 (confirmation only, on draft 5): ___ claimed, ___ reproduced, resolved or carried to v0.1: ___.
